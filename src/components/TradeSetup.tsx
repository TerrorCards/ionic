...
class TradeSetup extends React.Component<props, state> {
  ...
  showTradeSummary =() => {
    ...
    return(
      <IonGrid>
      <IonRow>
        <IonCol class="ion-text-center">You are giving to {this.props.otherUser}</IonCol>
      </IonRow>
      <IonRow>
      ...
      <IonRow>
          <IonCol class="ion-text-center">You are receiving from  {this.props.otherUser}</IonCol>
      </IonRow>
      <IonRow>
  ...
  generateImageSelectionList =() => {
    ...
    personList.map((ch:any,i:number) => {
      ...
      ch.map((c:any, z:number) => {
        ...
        item.push(
          <div key={c.ID} style={{width:cardWidth+"%", position:"relative", margin:"auto"}}>
        ...
        if(i === (personList.length -1)){
          if(ch.length  < this.state.layoutState.layoutCount) {
            ...
            for(let a=0; a < remainder; a++) {
              item.push(
                <div key={a+"_"+z} style={{display:"flex",  width:cardWidth+"%", margin:"auto"}}></div>
              )             
            }
          }
        }
      ...
      list.push(
        <div key={i} style={{display:"flex", flexDirection:"row"}}>{item}</div>
      );
    });
